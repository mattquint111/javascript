...
// arrays for menu item objects
let starterDishes = dishes.filter((item) => item.course === "Starters");
let entreeDishes = dishes.filter((item) => item.course === "Entrees");
let dessertDishes = dishes.filter((item) => item.course === "Desserts");

// starter menu
let startersMenuItems = document.getElementById("startersMenuItems")
// append items to appropriate <ul>
starterDishes.forEach((item) => {
  let li = `
  <li class="menuItem starterItem">
  <img
    src="${item.imageURL}"
    class="itemImage"
    height="100"
  ...
  />
  <div class="itemText">
    <label class="itemTitle"><b>${item.title}</b></label>
    <label class="itemDescription"
      ><em>${item.description}</em></label
    >
  </div>
...
startersMenuItems.insertAdjacentHTML("beforeend", li)
});

// entree menu
let entreesMenuItems = document.getElementById("entreesMenuItems")
// append items to appropriate <ul>
entreeDishes.forEach((item) => {
  let li = `
  <li class="menuItem entreeItem">
  <img
    src="${item.imageURL}"
    class="itemImage"
    height="100"
...
entreesMenuItems.insertAdjacentHTML("beforeend", li)
});

// dessert menu
let dessertsMenuItems = document.getElementById("dessertsMenuItems")
// append items to appropriate <ul>
dessertDishes.forEach((item) => {
  let li = `
  <li class="menuItem dessertItem">
  <img
    src="${item.imageURL}"
    class="itemImage"
    height="100"
...
dessertsMenuItems.insertAdjacentHTML("beforeend", li)
});